...
$result = $conn->query("SELECT MAX(Norder) AS max_id FROM depart");
...
$next_id = $row['max_id'] + 1;
 ...
 if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO depart ( Date, Destination, dossier, montant, reference, date_courrier, contact, Resume, Classement, Type_courrier, Origine, Objet_courrier, A_suivre, Piece_jointe,user_id) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
